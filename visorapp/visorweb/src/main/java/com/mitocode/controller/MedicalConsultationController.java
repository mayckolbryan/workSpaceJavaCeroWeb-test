...
public class MedicalConsultationController implements Serializable{
	...
	public void saveMedicalConsultation() {
		try {
			if (patient != null) {
				if (!detailConsultations.isEmpty()) {
					User user = new User();
...
//					doctor = doctorService.findById(doctor);
					...
					medicalConsultationService.insert(medicalConsultation);

					resetForm();
					cleanDetailConsultation();

					Message.messageInfo("Registro de Consulta Médica");
				} else {
					Message.messageInfo("La Consulta Médica no tiene detalle");
				}
			} else {
